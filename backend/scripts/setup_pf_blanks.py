...
«Склейка МДФ с заготовкой → Фрезеровка → Окутка»:
  ...
  • партии детали не трогаются: на «Фрезеровке» и «Окутке» они уже
    отфрезерованы; на «Склейке» их быть не должно — иначе стоп.
...
for (group_id, size), parts in sorted(by_blank.items(), key=lambda kv: (groups[kv[0][0]].name, kv[0][1])):
    ...
    for part in parts:
        ...
        # Партии на «Фрезеровке» уже отфрезерованы (сказал пользователь) —
        # остаются у детали. На «Склейке» партий быть не должно (их удаляют
        # отдельно, по решению пользователя) — иначе стоп, ничего не пишем.
        if glue is not None:
            left = db.query(PartUnit).filter(PartUnit.part_id == part.id, PartUnit.stage_id == glue.id).all()
            if left:
                raise SystemExit(f"«{part.name}»: на этапе «{GLUE}» партии {[u.id for u in left]} — сначала решите, что с ними")
...
print(f"\nзаготовок заведено: {created} (всего {len(by_blank)}), деталей связано: {linked}, партий не трогали")
if skipped:
