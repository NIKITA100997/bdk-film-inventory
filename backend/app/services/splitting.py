...
def cut_to_length(unit: MaterialUnit, cut_length_m: float, *, remainder_location: str | None = None) -> SplitOutcome:
    """Раскрой (по длине) — уменьшает длину единицы на месте: на складе
    (единица "На хранении") либо уже выданной участку (списание фактически
    израсходованного метража во время смены, любой участок). Отрезанный
    кусок точного размера уходит в производство сразу (списывается, новая
    единица не создаётся) — остаток по длине продолжает жить под тем же ID.
    """
