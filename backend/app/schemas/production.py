...
class ProductionTaskLineIssuedUnitOut(BaseModel):
    ...
    # различает реальный возврат (return_unit явно очищает area) от
    # "На_хранении, но ждёт довыдачи участку" после приёмки на хабе
    # (receive_transfer_line area не трогает) — обе ситуации иначе
    # неотличимы по одному только status.
    area: str | None
...
class ProductionTaskLineOut(BaseModel):
    ...
    # независимый от is_closed ручной флаг: строку можно закрыть по
    # выдаче, а по производству она ещё продолжается, и наоборот. Не
    # зависит от remaining_pieces — рост quantity_pieces (план
    # пересмотрели) не переоткрывает автоматически.
