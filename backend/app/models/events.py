...
class EventType(str, enum.Enum):
    ...
    # (POST /units/{id}/adjust) вместо правки истории напрямую в БД:
    # единственный тип события, который создаёт этот путь. write_off_
    # reason здесь не используется (это не списание) — причина/заметка
    # кладутся в write_off_note (переиспользуем существующее поле, не
    # заводим отдельное ради одного этого случая).
