...
class WriteOffReasonEntry(Base):
    ...
    category — "warehouse"/"production"/"general"/"parts" (раздел про
    модуль "Брак и списания"): чем пикер причины на списании единицы
    фильтрует список (warehouse+general), чем отчёт о браке на
    производстве — отдельно (production+general), и чем списание партии
    деталей (parts+general, раздел про физический учёт деталей — причины
    брака плёнки к дереву/МДФ не подходят). Одна таблица вместо
    нескольких — решает открытый вопрос бэклога (п. 12.6) без раздвоения
    справочника."""
