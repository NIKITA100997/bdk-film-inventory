...
export interface ProductionTaskLineIssuedUnit {
  ...
  is_strip: boolean;
  // Раздел про разбор задания единой таблицей — "Выдан_участку" / "В_
  // перемещении" (едет через хаб) / "На_хранении" (хаб уже принял, ждёт
  // довыдачи участку локально на том складе) — тот же кусок в разных
  // точках своего пути к участку.
  status: string;
...
export interface ProductionTaskLineReportCreate {
  ...
  // ("spisat"), как раньше; "pererabotka" резервирует брак (партия
...
export interface NaryadParsedLine {
  ...
  // посчитанная по профилю (ширина+глубина из текста названия); для
  // остальных деталей — точное значение из найденного соответствия в
  // справочнике (см. suggested_part_id), либо null, если совпадения нет
  // — тогда досчитывается стандартно при создании (calc_default_strip_width).
  strip_width_mm: number | null;
  // Раздел про соответствие деталям у наряд-заказа — id найденной в
  // справочнике детали при уверенном совпадении; сейчас не используется
