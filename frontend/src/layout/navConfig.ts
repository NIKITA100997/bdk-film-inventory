...
export const navTree: NavBlock[] = [
  ...
  },
  {
    ...
    items: [
      ...
      // резка/ABC/закупки), ему логичнее быть рядом со "Стеллажами", не
      // отдельно. Экран внутри уже объединён с картой стеллажей
      // (переключатель "Список/Карта", см. MaterialInventory.tsx) — оба
      // пункта меню (этот и следующий) по-прежнему ведут на один и тот же
      // экран, просто открывают разный вид по умолчанию. Названия — раздел
      // про то, что "Стеллажи и полки" перестало честно описывать экран
      // после слияния с "Остатками" — теперь явно один и тот же концепт
      // ("Остатки плёнки"), различаются только видом (список/карта), а не
      // два разных места в приложении со случайно похожей темой.
      { key: "stock", path: "/stock", label: "Остатки плёнки (список)" },
      { key: "storage", path: "/storage", label: "Остатки плёнки (карта)" },
      {
