...
export const navTree: NavBlock[] = [
  {
    ...
    items: [
      ...
      // "что и сколько у нас есть вообще" по всем доменам сразу (сейчас
      // плёнка + п/ф, дальше — ГП/материалы), отдельно от рабочих экранов
      // в "Материалах"/"Деталях" — те остаются с действиями для
      // кладовщиков/мастеров, этот — сводка для планирования. Раньше
      // здесь же дублировалась чисто плёночная "Остатки плёнки" — с
      // появлением этого настоящего кросс-доменного справочника её
  ...
  },
  {
    ...
    items: [
      ...
      // Переехала из "Основного" (была там как "сквозная справка для всех
      // ролей" — эту роль теперь несёт "Общие остатки" выше) — по факту
      // плёночно-специфичный рабочий экран кладовщика (приёмка/списание/
      // резка/ABC/закупки). Раньше здесь же по соседству был ещё и
      // отдельный пункт "Стеллажи и полки" на тот же экран (MaterialInventory,
      // переключатель "Список/Карта" внутри) — два похожих пункта меню на
      // один и тот же экран выглядели уже не подстраховкой привычки, а
      // просто дублем; один пункт, тот же приём, что уже у "Остатки и
      // стеллажи п/ф" ниже. /storage как ОТДЕЛЬНЫЙ РОУТ по-прежнему
      // работает (сканирование "Р-.../СШ..." в unitSearch.ts открывает
      // именно его, с видом "Карта") — просто у него больше нет своего
      // пункта в меню, только у /stock.
