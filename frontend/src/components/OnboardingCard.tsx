...
const HELP_BLOCKS: HelpBlock[] = [
  {
    ...
    body: "«Материалы (плёнка) → Приёмка плёнки» — заполните рулон и нажмите «Добавить и дальше», ячейка подбирается автоматически. После приёмки — печать этикеток одной кнопкой.",
    action: { label: "Открыть приёмку", path: "/m/receive" },
  },
  {
    test: (u) => u.permissions.includes("units.issue"),
    title: "Выдача участку",
    body: "«Материалы (плёнка) → Выдача участку» — потребности по заданиям подставляются автоматически, донор на разрезку подбирается сам.",
  ...
  },
  {
    ...
    body: "«Материалы (плёнка) → Закупки плёнки» — заявки поставщику, привязка к приёмке при закрытии.",
